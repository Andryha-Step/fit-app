...
export default function TabSwitcher(props: TabSwitcherProps): JSX.Element {
    ...
    >
        {props.tabs?.map(tab => customTab ? customTab({
...
export function useTabSwitcher(tabSwitcherArgs: TabSwitcherHookArgs): TabSwitcherHook {
    ...
    const tabSwitcherProps = {
        ...
        visualStyle: visualStyle,
        layoutStyle: layoutStyle,
        withScroll, borderIndicatior, showTitleOnlyWhenActive, fontWeight, fontSize
    }
...
const StyledTab = styled.div<{ borderIndicatior?: boolean, fontSize?: string, fontWeight?: string, layoutStyle?: TabSwitcherLayoutStyle, visualStyle?: TabSwitcherVisualStyle, active?: boolean, withScroll?: boolean }>`
    display: flex;
    justify-content: center;
    align-items: center;
    ...
    ${p => p.visualStyle === 'button' && p.active ? `
...
const StyledTitle = styled(Title) <{
    ...
    ${p => p.showTitleOnlyWhenActive && !p.active ? `
        opacity: 0;
    ` : ''}
